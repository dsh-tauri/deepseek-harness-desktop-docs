...
panel entry, Skills/MCP management, scheduled tasks, turn-level undo, desktop pets, and
context menus. Scheduled tasks run in independent Agent sessions; Turn Rewind uses
private Git snapshots without changing the user's repository history. The Windows
Minimal-mode repair is downloaded from its GitHub repository when selected; its source
